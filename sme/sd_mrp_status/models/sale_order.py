...
class SaleOrderLine(models.Model):
    ...
    def _compute_manufactured_qty(self):
        for line in self:
            ...
            if line.product_id.type in ['product', 'consu']:
                # mos = self.env['procurement.group'].search([('sale_id', '=', line.order_id.id), ('mrp_production_ids', '!=', False)]).mapped('mrp_production_ids')
                # if not mos:
        ...
        for line in self:
            ...
            if line.product_id.type in ['product', 'consu']:
                if line.state not in ['draft', 'cancel']:
                    # mos = line.order_id.procurement_group_id.stock_move_ids.created_production_id.procurement_group_id.mrp_production_ids
                    mos = self.env['mrp.production'].search([('origin', '=', line.order_id.name)])
                    if mos:
