odoo.define("sd_pos_stripe_payment_terminal.payment", function(require) {
    ...
    function connectReaderHandler(is_test, location_id) {
        ...
        return terminal.discoverReaders(config).then(function(discoverResult) {
            if (discoverResult.error) {
                ...
                return terminal.connectReader(selectedReader, { fail_if_in_use: true }).then(function(connectResult) {
    ...
    var StripePaymentTerminal = PaymentInterface.extend({
            ...
            return rpc.query({
                ...
                if (result == "invalid_key") {
                    ...
                    simulate_card_response = false;
                     self._show_error(_t('Authentication failed. Please check your Stripe credentials.'));
        ...
        send_payment_cancel: function(order, cid) {
            ...
            if (is_connected){ this._sd_pos_stripe_payment_terminal_cancel();}
        ...
        paymentIntent: function() {
            ...
            if (line && line.payment_method.is_simulated_reader) {
            ...
            return ajax.jsonRpc("/pos_payment_intent", "call", {
                    amount: real_amount,
                    payment_method_id: payment_method_id,
                    currency: currency,
                })
                ...
                .then(function(data) {
                    if (data.error) {
                        self._show_error(_t("Sorry! Could not Intent Payment. " + data.error));
        ...
        async connection_stripe(data, operation) {

            return await ajax.jsonRpc("/pos_connection_token", "call", {
                    payment_method_id: payment_method_id,
                })
                .then(function(response) {
        ...
        _sd_pos_stripe_payment_terminal_common_message_header: function() {
            ...
            return {
                ...
                POIID: this.payment_method.sd_pos_stripe_payment_terminal_terminal_identifier,
            };
        },
        ...
        _sd_pos_stripe_payment_terminal_pay_data: function() {
            ...
            var data = {
                SaleToPOIRequest: {
                    MessageHeader: _.extend(this._sd_pos_stripe_payment_terminal_common_message_header(), {
...
//            return Promise.reject()
                                    ...
                                    return terminal
                                        ...
                                        .then(function(result) {
                                            ...
                                            if (result.error) {
                                                ...
                                                self._show_error(_t("Error while Process payment. "+result.error.message));
                                                // Placeholder for handling result.error
                                                if (self.remaining_polls != 0) {
                                                    self.remaining_polls--;
                                                } else {
                                                ...
                                                result.paymentIntent.charges.data[0].payment_method_details.type).then(
                                                    function(response) {
                                                        ...
                                                        line.stripe_intend_payment_id = result.paymentIntent.id;
        ...
        _sd_pos_stripe_payment_terminal_handle_response: function(response) {
            ...
            if (response.error) {
                this._show_error(_t('Authentication failed. Please check your Stripe credentials.'));
        ...
        this.was_cancelled = !!this.polling;
            ...
            if (!title) {
                title = _t("Stripe Payment Terminal Error");
            }
