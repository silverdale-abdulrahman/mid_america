...
class AccountAnalyticLine(models.Model):
    ...
    def _compute_so_line(self):
        # non_billed_helpdesk_timesheets = self.filtered(lambda t: not t.is_so_line_edited and t.helpdesk_ticket_id)
        non_billed_timesheets = self.filtered(lambda t: not t.is_so_line_edited)
    ...
    def onchange_non_billable_timesheet_linea(self):
        ...
        for record in self:
            if record.validated is False:
                ...
                if record.validated and record.non_billable:
